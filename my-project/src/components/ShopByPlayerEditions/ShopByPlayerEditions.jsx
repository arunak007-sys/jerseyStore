...
import '../ShopByPlayerEditions/ShopByPlayerEditions.css'
...
export default function ShopByPlayerEditions() {
  ...
  const { playerEditions, setPlayerEditions,searchItem,setItemSearch,SetSearchItem,searchQuery, setLikeProducts, cartProducts, setCartProducts, myJersey, prodDisp, setProdDdisp, setSearchQuery, setMyJersey, signIn, setSignIn,
     likeProducts
  } = useContext(myContext)

  useEffect(() => {
    handlePlayerEditions()
  }, [])

  function handlePlayerEditions() {

    setPlayerEditions(myJersey.filter(data => data.category == "player Editions"))
  }
...
function serachInp(e){
  ...
  return (
    <div className="main">
      ...
      <div className="centerc3">
        <div className="imagec33">
          <img src="https://dukaan.b-cdn.net/1440x1440/webp/upload_file_service/326e634b-0427-42e9-842f-86626359d9d9/image.png" alt="" />
        </div>
        <div className="imageTitlec3"><p>Player Editions <h5 >({playerEditions.length} items)</h5></p></div>
        <div><p style={{ color: 'grey', letterSpacing: 1 }}><Link to={'/'} style={{ textDecoration: 'none', color: 'black' }}>Home</Link> &nbsp;/ &nbsp;Player Editions</p></div>

        <div className="center-items3">
