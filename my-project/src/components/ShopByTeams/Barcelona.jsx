...
export default function Barcelona() {
    ...
    const { barcelona,setBarcelona,likeProducts, setLikeProducts, cartProducts, setCartProducts, myJersey, prodDisp, setProdDdisp, setSearchQuery, signIn, setSignIn,
    ...
    useEffect(() => {
        handleShopByBarcelona()
      }, [])
    
      function handleShopByBarcelona() {
    
        setBarcelona(myJersey.filter(data => data.specialCategory == "Barcelona"))
      }
...
<Nav.Link style={{ fontWeight: 'bold' }} >Call Us : +91-9746726000</Nav.Link>
            ...
            <div className="centerc5">
                <div className="imagec">
                    <img src="https://dms.mydukaan.io/original/webp/media/0a3d7bde-f4d1-49cb-8dc5-840a4f3ed2ee.blob" alt="" />
                </div>
                <div className="imageTitlec"><p>Shop By Teams<h5 >({barcelona.length} items)</h5></p></div>
                <div><p style={{ color: 'grey', letterSpacing: 1 }}><Link to={'/'} style={{ textDecoration: 'none', color: 'black' }}>Home</Link> &nbsp;/ &nbsp;FC Barcelona</p></div>
