...
export default function Liverpool() {
    ...
    const { likeProducts, setLikeProducts, cartProducts, setCartProducts, myJersey, prodDisp, setProdDdisp, setSearchQuery, signIn, setSignIn,
       liverpool,setLiverpool,searchItem,setItemSearch,searchQuery,SetSearchItem } = useContext(myContext)
    ...
    useEffect(() => {
        handleShopByLiverpool()
      }, [])
    
      function handleShopByLiverpool() {
    
        setLiverpool(myJersey.filter(data => data.specialCategory == "Liverpool"))
      }
...
<Nav.Link style={{ fontWeight: 'bold' }} >Call Us : +91-9746726000</Nav.Link>
            ...
            <div className="centerc5">
                <div className="imagec">
                    <img src="https://dms.mydukaan.io/original/webp/media/a8b08d6a-2916-4793-bd1f-669767a47a7b.blob" alt="" />
                </div>
                <div className="imageTitlec"><p>Shop By Teams<h5 >({liverpool.length} items)</h5></p></div>
                <div><p style={{ color: 'grey', letterSpacing: 1 }}><Link to={'/'} style={{ textDecoration: 'none', color: 'black' }}>Home</Link> &nbsp;/ &nbsp;Liverpool</p></div>
