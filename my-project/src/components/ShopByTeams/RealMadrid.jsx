...
export default function RealMadrid() {
    ...
    const {likeProducts, setLikeProducts, cartProducts, setCartProducts, myJersey, prodDisp, setProdDdisp, setSearchQuery,  signIn, setSignIn,
      realMadrid,setRealMadrid,searchItem,setItemSearch,searchQuery,SetSearchItem } = useContext(myContext)
    ...
    useEffect(() => {
        handleShopByRealMadrid()
      }, [])
    
      function handleShopByRealMadrid() {
    
        setRealMadrid(myJersey.filter(data => data.specialCategory == "RealMadrid"))
      }
...
<Nav.Link style={{ fontWeight: 'bold' }} >Call Us : +91-9746726000</Nav.Link>
            ...
            <div className="centerc5">
                <div className="imagec">
                    <img src="https://dms.mydukaan.io/original/webp/media/e3b83e4a-962b-49bc-9abd-3c22b7ac97ea.blob" alt="" />
                </div>
                <div className="imageTitlec"><p>Shop By Teams<h5 >({realMadrid.length} items)</h5></p></div>
                <div><p style={{ color: 'grey', letterSpacing: 1 }}><Link to={'/'} style={{ textDecoration: 'none', color: 'black' }}>Home</Link> &nbsp;/ &nbsp;Real Madrid</p></div>
