...
export default function Alnassar() {
    ...
    const {alnassar,setAlnassar,likeProducts, setLikeProducts, cartProducts, setCartProducts, myJersey, prodDisp, setProdDdisp, setSearchQuery,  signIn, setSignIn,
    ...
    useEffect(() => {
        handleShopByAlnassar()
      }, [])
    
      function handleShopByAlnassar() {
    
        setAlnassar(myJersey.filter(data => data.specialCategory == "Alnassar"))
      }
    ...
    <button className="cart-count">{cartProducts.length}</button>
            ...
            <div className="centerc5">
                <div className="imagec">
                    <img src="https://dms.mydukaan.io/original/webp/media/fd83cd52-623a-4b7c-a070-98e7e03a6aad.blob" alt="" />
                </div>
                <div className="imageTitlec"><p>Shop By Teams<h5 >({alnassar.length} items)</h5></p></div>
                <div><p style={{ color: 'grey', letterSpacing: 1 }}><Link to={'/'} style={{ textDecoration: 'none', color: 'black' }}>Home</Link> &nbsp;/ &nbsp;Al-Nassar</p></div>
