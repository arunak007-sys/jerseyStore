...
export default function ManchesterUnited() {
    ...
    const { likeProducts, setLikeProducts, cartProducts, setCartProducts, myJersey, prodDisp, setProdDdisp, setSearchQuery,  signIn, setSignIn,
      manchesterUnited,setManchesterUnited,searchItem,setItemSearch,searchQuery,SetSearchItem } = useContext(myContext)
    ...
    useEffect(() => {
        handleShopByManchesterUnited()
      }, [])
    
      function handleShopByManchesterUnited() {
    
        setManchesterUnited(myJersey.filter(data => data.specialCategory == "ManchesterUnited"))
      }
...
<Nav.Link style={{ fontWeight: 'bold' }} >Call Us : +91-9746726000</Nav.Link>
            ...
            <div className="centerc5">
                <div className="imagec">
                    <img src="https://dms.mydukaan.io/original/webp/media/7c742bbd-637c-4698-80fd-a0ab3e1bb207.blob" alt="" />
                </div>
                <div className="imageTitlec"><p>Shop By Teams<h5 >({manchesterUnited.length} items)</h5></p></div>
                <div><p style={{ color: 'grey', letterSpacing: 1 }}><Link to={'/'} style={{ textDecoration: 'none', color: 'black' }}>Home</Link> &nbsp;/ &nbsp;Manchester United</p></div>
          ...
          <div className="amain-card05">
            ...
            {
              manchesterUnited.map((data, index) => (
